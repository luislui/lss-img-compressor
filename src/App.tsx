...
function App() {
  ...
  const [maxDimension, setMaxDimension] = useState<number | undefined>(undefined) // Optional limit
  ...
  const compressSingleItem = async (
    ...
    try {
      ...
      const compressedResult = await compressImage(item.file, qualityVal, dimensionVal, scalePercentVal)
      return {
        ...item,
        status: 'success',
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    for (const item of newItems) {
      ...
      const updated = await compressSingleItem(item, quality, maxDimension, scalePercent)
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-450">
                Optimiza tus imágenes reduciendo su peso y convirtiéndolas a JPG.
              </p>
            </div>
          </div>
        ...
        >
          <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-lg border border-neutral-300 bg-white shadow-xl dark:border-neutral-600 dark:bg-neutral-800">
            ...
            <div className="space-y-4 px-5 py-4 text-sm text-neutral-700 dark:text-neutral-300">
              <section>
                <h3 className="mb-1 font-semibold text-neutral-900 dark:text-neutral-100">Carga Masiva</h3>
                <p className="mb-0">
                  Arrastra o selecciona múltiples imágenes (formatos JPG, PNG, WEBP, GIF, BMP) a la zona superior.
                  El procesamiento iniciará automáticamente usando la calidad de compresión configurada actualmente.
                </p>
              </section>

              <section>
                <h3 className="mb-1 font-semibold text-neutral-900 dark:text-neutral-100">Conversión a JPG</h3>
                <p className="mb-0">
                  Toda imagen cargada se convertirá automáticamente al formato estándar `.jpg` para una máxima
                  compatibilidad web y eficiencia de almacenamiento. Las transparencias se rellenan con fondo blanco.
                </p>
              </section>

              <section>
                <h3 className="mb-1 font-semibold text-neutral-900 dark:text-neutral-100">Controles de Calidad y Resolución</h3>
                <p className="mb-2">
                  Ajusta la calidad de compresión (10%-100%) y/o limita la resolución máxima de salida. Para aplicar
                  estos ajustes a las imágenes ya cargadas, haz clic en el botón <strong>&quot;Procesar Cambios&quot;</strong>.
                </p>
              </section>

              <section>
                <h3 className="mb-1 font-semibold text-neutral-900 dark:text-neutral-100">Privacidad y Procesamiento</h3>
                <p className="mb-0 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-amber-800 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-200 flex gap-2">
                  ...
                  <span>
                    <strong>100% Seguro:</strong> La compresión se realiza íntegramente de forma local en tu navegador.
                    Tus imágenes nunca se envían a ningún servidor web.
                  </span>
                </p>
              </section>
            </div>
